// @ts-nocheck
import { exportTemplate } from '../../src/tools/export-template';
import { ValidationError as InputValidationError } from '../../src/validation/errors';
...
jest.mock('../../src/validation/common', () => ({
  safeFilePath: jest.fn(async (value: string) => value),
}));
...
const exporterModule: any = jest.requireMock('../../src/import-export/template-exporter');
const middlewareModule: any = jest.requireMock('../../src/validation/middleware');
const commonModule: any = jest.requireMock('../../src/validation/common');
const errorHandlerModule: any = jest.requireMock('../../src/errors/handler');
...
describe('exportTemplate core behaviour', () => {
  beforeEach(() => {
    jest.resetAllMocks();
    commonModule.safeFilePath = jest.fn(async (value: string) => value);
    middlewareModule.validateAndSanitize = jest.fn();
    exporterModule.TemplateExporter = jest.fn();
    errorHandlerModule.ErrorHandler.handle = jest.fn((error: unknown) => error);
  ...
  const mockValidatedParams = (overrides: Partial<any> = {}) => ({
    template,
